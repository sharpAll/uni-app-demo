		...
		<!-- #ifndef H5 -->
			<view class="third-wapper">
				...
				<view class="third-icos-wapper">
					...
					<!-- #endif -->
					<!-- #ifdef MP-WEIXIN -->
					<!-- open-type是button组件的属性，用于获取第三方的开放能力 -->
					...
					success(loginRes) {
						...
						uni.getUserInfo({
							...
							success(info) {
								...
								// 调用开发者后台，执行一键注册或登录
								uni.request({
									url: me.serverUrl + "/appUnionLogin/" + logintype+'?qq='+me.serverKey,
									data: {
			...
			wxLogin(e) {
				...
				uni.login({
					...
					success(loginResult) {
						...
						uni.request({
							url: me.serverUrl + "/mpWXLogin/" + code+'?qq='+this.serverKey,
							data: {
...
// 				console.log(password);
				...
				uni.request({
					url: serverUrl + '/user/registOrLogin?qq='+this.serverKey,
					data: {
						"username": username,
						"password": password
					},
