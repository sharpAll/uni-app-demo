<template>
	<view class="page">
		...
		<view class="scroll-block">
			...
			<scroll-view scroll-x class="scroll-list">
				<image v-for="(img, imgIndex) in plotPicsArray" :src="img" class="plot-image" mode="aspectFill" @click="lookPlots"
...
<script>
	...
	export default {
		...
		onLoad(params) {
			...
			// 获取预告片的详细信息
			uni.request({
				url: this.serverUrl + '/search/trailer/' + trailerId + '?qq=' + this.serverKey,
				method: "POST",
				success: (res) => {
			...
			// 获取导演
			uni.request({
				url: this.serverUrl + '/search/staff/' + trailerId + '/1' + '?qq=' + this.serverKey,
				method: "POST",
				success: (res) => {
					// 获取真实数据之前，务必判断状态是否为200
					if (res.data.status == 200) {
			...
			// 获取获取演员
			uni.request({
				url: this.serverUrl + '/search/staff/' + trailerId + '/2' + '?qq=' + this.serverKey,
				method: "POST",
				success: (res) => {
					// 获取真实数据之前，务必判断状态是否为200
					if (res.data.status == 200) {
