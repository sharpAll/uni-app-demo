	...
	export default {
		...
		methods: {
						...
						if (index == 0) {
							...
							uni.chooseImage({
								...
								success(res) {
									...
									// #ifdef H5
									uni.navigateTo({
										url: "../meFace/meFace?tempFilePath=" + tempFilePath+'&qq='+me.serverKey
									})
									// #endif
									// #ifndef H5
									uni.navigateTo({
										url: "../faceCrop/faceCrop?tempFilePath=" + tempFilePath+'&qq='+me.serverKey
									})
									// #endif
								}
							})
			...
			logout() {
				...
				uni.request({
					url: serverUrl + '/user/logout?userId=' + globalUser.id+'&qq='+this.serverKey,
					method: "POST",
					success: (res) => {
